...
<<<<<<< .mine
    networkMgr.register(ProtocolIDLua
||||||| .r21
=======
...
--Socket消息--
function Network.OnSocket(type,data)
    Event.Brocast(tostring(type), data);
end
...
--登录返回--
function Network.OnMessage2(buffer) 
...
--PBC——lua登录--
function Network.LoginSproto(buffer)
    ...
    local msg = logintest_pb.LoginTestRequest();
    msg:ParseFromString(data);
    
	log('TestLoginPblua: protocal:>'..protocal..' msg:>'..msg.id);
    log('TestLoginPbc: protocal:>'..protocal);
    
    --for _,v in ipairs(msg.infoDic) do
        --print("\t"..v.intdex, v.name)
    --end
end
