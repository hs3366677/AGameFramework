...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleInspectTab
    {
        ...
        private void OnGUIEditor()
        {
            ...
            if (GUILayout.Button("Load to Scene", GUILayout.MaxWidth(105f)))
            {
                Load2Scene();
            }
        ...
        private void Load2Scene()
        {
            ...
            if (currentSelectedBundle != null)
            {
                UnityEngine.Object[] objs = currentSelectedBundle.LoadAllAssets();

                Array.Sort(objs, delegate(Object obj1, Object obj2)
                {
                    if (obj1.GetType() == typeof(GameObject) && obj2.GetType() != typeof(GameObject))
                        return 1;
                    else if (obj1.GetType() != typeof(GameObject) && obj2.GetType() == typeof(GameObject))
                        return -1;
                    else
                        return 0;
                });
        ...
        private AssetBundle currentSelectedBundle;
        ...
        internal void SetBundleItem(IList<InspectTreeItem> selected)
        {
            ...
            {
                currentSelectedBundle = LoadBundle(selected[0].bundlePath);
                m_SingleInspector.SetBundle(currentSelectedBundle, selected[0].bundlePath, m_Data, this);
            }
            else
            {
                m_SingleInspector.SetBundle(null);
