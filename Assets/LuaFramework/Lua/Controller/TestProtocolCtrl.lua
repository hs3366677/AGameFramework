...
function TestProtocolCtrl.Awake()
	...
	panelMgr:CreatePanel('TestProtocol', this.OnCreate);
      ...
      --Protocal.CMD_LUA_EVENT_CALLBACK
    
    networkMgr:RegisterShortCallBack(Protocal.CMD_LUA_EVENT_CALLBACK, this.OnProtocolCallback);
    print('LuaFramework InitOK11111111111111--->>>');
end

function TestProtocolCtrl.OnProtocolCallback(_proto)
    print("TestProtocolCtrl.OnProtocolCallback");
--	Event.AddListener(Protocal.CMD_LUA_EVENT, this.OnMessage2); 
end
...
function TestProtocolCtrl:OnTestClick(go)
    ...
    buffer:WriteShort(Protocal.CMD_LUA_EVENT);
    buffer:WriteBuffer(msg);
    --networkMgr:SendMessage(buffer);
	

	networkMgr:SendTcpClientMessage(Protocal.CMD_LUA_EVENT, buffer); 
end
